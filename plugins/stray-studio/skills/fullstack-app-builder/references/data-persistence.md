...
2. Enforce invariants at the storage boundary.
   ...
   - When row security enforces tenant access, test the deployed request role and connection path. PostgreSQL superusers and `BYPASSRLS` roles bypass RLS; table owners normally do too unless forced. Re-establish tenant context for each transaction and test pooled-connection reuse. Use [identity-access.md](identity-access.md) for the access policy. See [PostgreSQL row security](https://www.postgresql.org/docs/18/ddl-rowsecurity.html) and [tenant-context scoping](https://cheatsheetseries.owasp.org/cheatsheets/Multi_Tenant_Security_Cheat_Sheet.html#scope-tenant-context-to-the-transaction).
