...
Carry the selected material-color plan into `prepare_asset_run.py --style-notes` (recorded in `style_contract`) and inspect the emitted base/row prompts before generation. For a flat-color diagnostic, explicitly name affected materials, sampled base colors and omitted shading; keep it in a separate small run with the matching frame count and references. The repair queue does not diagnose color drift or create this probe automatically. Keep raw/cleaned/final evidence and material verdicts in the visual QA note; no bundled option supplies semantic masks or material-specific recoloring.
...
Include playback intent in `--motion-beats`, for example `"loop: energetic run in place; landing compression, chest rising at push-off, flight, opposite stride; shoulder/hip counter-rotation and delayed hair swing"` or `"single action: crouch, jump to apex, descend, land; preserve jump height"`. The motion text is also recorded in `animation.motion_beats`. Pass `--registration fixed` for stationary idle/blink with planted feet, `--registration free` for locomotion (including run-in-place), travel or jumps, or leave it `unspecified` when unknown. Only `fixed` enables the automated contact-drift warning. Use `--style-notes` for any specific scale, orientation, or baseline constraints. Check generated prompt files before sending them; `prepare_asset_run.py` builds its own prompts rather than reading the Markdown templates. Free registration omits stationary lock instructions; complete the action-specific whole-body plan from `prompt-templates.md`.
...
3. Generate each ready job with the installed image generation capability.
   ...
   - Before filling a locomotion row, visually verify both sides' contact and passing/support key poses as described in `prompt-templates.md`. Preserve these source images and review notes. This is a manual generation/review stage, not an implemented key-pose scheduler; a partial pose preview must not be recorded as a completed full-row job.
...
For color mottling or shading flicker, apply the material-color plan in `prompt-templates.md` and the shading checks in `qa-rules.md` before accepting these exports. The bundled scripts do not recognize material regions or enforce shadow placement across poses; palette mapping and extraction reports cannot certify this. Use the installed image-editing capability for a paint repair, preserve originals, and record the current visual verdict separately.
