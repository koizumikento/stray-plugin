#!/usr/bin/env python3
"""Secondary image generation fallback for pixel-art asset jobs."""
...
def select_jobs(manifest: dict[str, object], job_ids: list[str]) -> list[dict[str, object]]:
    if job_ids:
        ...
        selected_ids = {str(job.get("id")) for job in manifest_jobs(manifest) if isinstance(job.get("id"), str)}
...
def main() -> None:
    ...
    for job in select_jobs(manifest, args.job_id):
        ...
        if deps:
            raise SystemExit(f"job {job_id} is not ready; missing dependency result(s): {', '.join(deps)}")
